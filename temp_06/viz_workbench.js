(function(){
  ...
  function drawClassic(){
    ...
    if (C.plotType==="scatter" || C.plotType==="poincare"){
      ...
      gClassic.lineWidth = Math.max(0.5, C.pointSize * 0.2);
      var gridSize = Math.floor(10 + C.gridN * 0.1);
      var stepSize = 2;
      
      // Create vector field from data
      var field = new Array(gridSize);
      for (var i = 0; i < gridSize; i++) {
        field[i] = new Array(gridSize);
        for (var j = 0; j < gridSize; j++) {
          field[i][j] = { vx: 0, vy: 0, count: 0 };
        }
      }
      ...
      for (var i = 0; i < rows.length; i++) {
        var r = rows[i];
        var gx = Math.floor(((r.x - b.xmin) / (b.xmax - b.xmin)) * (gridSize - 1));
        var gy = Math.floor(((r.y - b.ymin) / (b.ymax - b.ymin)) * (gridSize - 1));
        
        if (gx >= 0 && gx < gridSize && gy >= 0 && gy < gridSize) {
          // Simple gradient based on position
          field[gx][gy].vx += (r.x - b.xmin) / (b.xmax - b.xmin) - 0.5;
          field[gx][gy].vy += (r.y - b.ymin) / (b.ymax - b.ymin) - 0.5;
          field[gx][gy].count++;
        }
      }
      ...
      for (var i = 0; i < gridSize; i++) {
        for (var j = 0; j < gridSize; j++) {
          if (field[i][j].count > 0) {
            field[i][j].vx /= field[i][j].count;
            field[i][j].vy /= field[i][j].count;
          }
        }
      }
      
      // Draw streamlines
      var numStreams = Math.floor(8 + C.artDetail * 4);
      var streamLength = Math.floor(20 + C.artIntensity * 30);
      ...
      for (var stream = 0; stream < numStreams; stream++) {
        ...
        gClassic.strokeStyle = colorFor(stream / numStreams, C.colorscale, C.opacity);
        gClassic.beginPath();
        ...
        for (var step = 0; step < streamLength; step++) {
          ...
          if (gx >= 0 && gx < gridSize - 1 && gy >= 0 && gy < gridSize - 1) {
            ...
            var v00 = field[gx][gy];
            var v10 = field[gx+1][gy];
            var v01 = field[gx][gy+1];
            var v11 = field[gx+1][gy+1];
            ...
            x += vx * stepSize * (C.pointSize / 18) * 10;
            y += vy * stepSize * (C.pointSize / 18) * 10;
  ...
  function updatePointSizeRange() {
    ...
    var highRangePlots = ["contour", "art-contours", "art-halftone", "art-rd"];
